...
| Signal sim workspaces | Local/test API env `INTERNAL_WORKSPACE_VIEWER_EMAILS` | Optional operator emails so `/signal-sim` can list test workspaces. Keep empty in production; production-like startup rejects this cross-workspace shortcut. |
