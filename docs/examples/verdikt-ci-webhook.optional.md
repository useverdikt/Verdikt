...
1. **`verdikt:rc` label** on the PR → Verdikt opens the cert window (`commit_sha` + `pr_number`) and **auto-pulls** connected integrations.
2. Verdikt pulls signals from Braintrust, BrowserStack, Sentry, Datadog, etc., or the agent calls **`post_signals`** via MCP.
