...
Post-verdict integration delivery is also being migrated in stages. `OUTBOX_MODE=shadow` (the default) writes deduplicated Slack, webhook, callback, and VCS delivery intents in the same transaction as the verdict. Existing delivery remains authoritative; no outbox row sends traffic yet. Set `OUTBOX_MODE=off` to disable intent recording immediately.
...
- **Service events** — Certification snapshot failures, escalation SLA breaches, collection-sweep claims, gate actions, post-verdict side effects, gate context build failures, and VCS monitor scan failures all emit structured lines via `src/lib/observability.js` (same `LOG_JSON=1` switch). Process-local counters (`cert_snapshot_*`, `escalation_sla_breach`, `collection_sweep_*`, `gate_action_*`, `post_verdict_*`, `gate_context_*`, `vcs_monitor_*`) are for debugging; rely on log aggregation across API/worker processes.
