...
Post-verdict integration delivery is also being migrated in stages. `OUTBOX_MODE=shadow` (the default) writes deduplicated Slack, webhook, callback, and VCS delivery intents in the same transaction as the verdict. A leased worker compares observable legacy webhook/VCS records, reports mismatches, and flags configured Slack/callback channels as unverifiable until they gain delivery records. It never sends external traffic. Existing delivery remains authoritative; set `OUTBOX_MODE=off` to disable recording and shadow processing.
