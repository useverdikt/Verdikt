...
VCS monitoring sweeps always acquire durable, owner-scoped database leases before calling GitHub or GitLab. This keeps worker replicas from scanning or ingesting evidence for the same window concurrently. `VCS_MONITOR_SWEEP_BATCH_SIZE` defaults to 20 and `VCS_MONITOR_SWEEP_CLAIM_LEASE_MS` defaults to 30 minutes; an expired lease is reclaimable after a worker exits.

Escalation SLA sweeps also lease overdue requests before appending breach audits or sending reminders. The breach state transition and audit append commit atomically, while owner-scoped reminder updates prevent concurrent sends. `ESCALATION_SLA_SWEEP_BATCH_SIZE` defaults to 100 and `ESCALATION_SLA_SWEEP_CLAIM_LEASE_MS` defaults to five minutes.

Certification-snapshot retries and startup backfills use separate owner-scoped database leases. Retry completion and rescheduling require the active owner, while newly queued verdict evidence invalidates an older claim so a stale worker cannot delete newer work. `CERT_SNAPSHOT_RETRY_CLAIM_LEASE_MS` defaults to two minutes and `CERT_SNAPSHOT_BACKFILL_CLAIM_LEASE_MS` defaults to five minutes.

Post-verdict integration delivery is also being migrated in stages. `OUTBOX_MODE=shadow` (the default) writes deduplicated delivery intents in the same transaction as verdicts and human overrides; override events record only the webhook and callback effects that their legacy path executes. Legacy Slack/callback delivery paths record fail-open, URL-free observations on those rows; the leased worker compares their delivery inputs alongside webhook/VCS evidence and reports mismatches. Payload equality is insufficient when the authoritative delivery returned non-2xx or recorded an error: that evidence is a mismatch. The worker never sends external traffic. Existing delivery remains authoritative; set `OUTBOX_MODE=off` to disable recording and shadow processing. Human workspace sessions can read aggregate cutover evidence from `GET /api/workspaces/:workspaceId/outbound-effects/readiness?window_days=7`; the response excludes payloads and destinations and fails closed on small samples, failed deliveries, or other unhealthy evidence.
