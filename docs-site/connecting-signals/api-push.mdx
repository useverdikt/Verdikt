...
This is the recommended path for custom eval stacks and push integrations (e.g. ZizkaDB, in-house metrics).
...
- `source` — your signal source id (e.g. `zizkadb`, `custom`)
    ...
    steps:
      ...
      - name: Run your eval
        ...
        run: |
          ...
          DRIFT=$(python run_eval.py --sha ${{ github.event.pull_request.head.sha }})
        ...
        run: |
          curl -sfS -X POST "${VERDIKT_API}/api/releases/${{ steps.gate.outputs.release_id }}/signals" \
            ...
            -d "{
              \"source\": \"zizkadb\",
              \"signals\": {
                \"behavioural_drift\": ${{ steps.eval.outputs.drift }},
                \"session_anomaly_rate\": 0.03
              }
            }"
```
...
await push_signals(
    release_id,
    source="zizkadb",
    signals={"behavioural_drift": 0.12, "session_anomaly_rate": 0.03},
)
```
