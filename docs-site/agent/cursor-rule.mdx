---
title: Cursor agent rule
description: Copy this rule into your project so coding agents know when to certify through Verdikt.
---

Save the block below as **`.cursor/rules/verdikt.mdc`** in your repo (create the `.cursor/rules` folder if needed). Cursor will apply it when certifying PRs through Verdikt.

```md
---
description: Certify GitHub PRs through Verdikt before merge — label trigger, signals, check_gate.action, escalate
alwaysApply: false
---
...
`create_release` → `post_signals` → `get_verdict` → `check_gate` → `escalate` | merge

Config: [MCP setup](/agent/mcp-setup) and **Settings → Agent access** in the Verdikt app.
```

See also [MCP setup](/agent/mcp-setup) and the [@useverdikt/mcp](https://www.npmjs.com/package/@useverdikt/mcp) package on npm for tool reference.
