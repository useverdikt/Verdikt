...
Copy the [Cursor agent rule](/agent/cursor-rule) into your project as `.cursor/rules/verdikt.mdc` so the agent calls **`release_brief`** when the gate is blocked — not just **`check_gate`** in a loop.
